...
void FacebookManager::requestReadPermissions(std::function<void(FacebookManager::PermissionState, std::string)> callback, bool)
{
    callback(PermissionState::ERROR, "Facebook integration is unavailable in this Android build.");
}

void FacebookManager::requestPublishPermissions(std::function<void(FacebookManager::PermissionState, std::string)> callback, bool)
{
    callback(PermissionState::ERROR, "Facebook integration is unavailable in this Android build.");
}
...
bool FacebookManager::hasPermission(std::string)
{
    return false;
}

void FacebookManager::graphRequest(std::string, const std::unordered_map<std::string, std::string> &,
                                   HTTPMethod, std::function<void(Value&&, std::string)> callback)
{
    callback(Value(), "Facebook integration is unavailable in this Android build.");
}
