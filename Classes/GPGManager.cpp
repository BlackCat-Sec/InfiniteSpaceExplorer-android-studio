...
#if CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID && __has_include(<gpg/gpg.h>)
#include <jni.h>
#include <string>
...
static GPGManager::SignStatus signStatus = GPGManager::SignStatus::PLATFORM_UNAVAILABLE;

void GPGManager::initialize()
{
    signStatus = GPGManager::SignStatus::PLATFORM_UNAVAILABLE;
}
...
void GPGManager::loadPlayerCurrentScore(std::function<void(const ScoreManager::ScoreData&)> handler)
{
    handler(ScoreManager::ScoreData(-1, "", 0));
}

void GPGManager::loadHighscoresOnRange(ScoreManager::SocialConstraint, ScoreManager::TimeConstraint,
                                       long, long,
                                       std::function<void(long, std::vector<ScoreManager::ScoreData>&&, std::string)> handler,
                                       bool)
{
    handler(-1, std::vector<ScoreManager::ScoreData>(), "Google Play Games is unavailable in this Android build.");
}

void GPGManager::reportScore(int64_t, ScoreManager::AdditionalContext)
{
}

void GPGManager::unlockAchievement(std::string)
{
}

void GPGManager::updateAchievementStatus(std::string, int)
{
}

void GPGManager::getAchievementProgress(std::string, std::function<void(int)> handler)
{
    handler(-1);
}
